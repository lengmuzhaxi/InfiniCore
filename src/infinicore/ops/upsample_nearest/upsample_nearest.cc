...
Tensor upsample_nearest(Tensor input, const std::vector<int64_t>& output_size) {
    ...
    // 校验
    if (ndim < 3 || ndim > 4) {
         // 为了稳健性，如果不是 3 或 4 维，暂时抛错，或者视具体需求放宽
         // 这里根据 PyTorch 习惯，upsample 通常支持 3D(Temporal), 4D(Spatial), 5D(Volumetric)
         // 我们目前支持 3D 和 4D
         if (ndim != 3 && ndim != 4) {
    ...
    if (ndim == 3) {
        ...
        if (output_size.size() == 1) {
            ...
            // 如果传了 2 个参数给 3D input，通常期望第一个是 1 (height) 或者被忽略?
            // PyTorch 的 interpolate 针对 3D 输入只接受 size 为 int 或 len=1 的 tuple
            // 但如果用户强行传了 len=2, 我们取最后一个作为 W
