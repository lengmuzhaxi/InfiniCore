...
public:
    ...
    static utils::Result<FloatPowerInfo> create(
        ...
        // 1. 允许 Mixed Precision (混合精度)
        // 我们不再检查 out_desc->dtype() == input_desc->dtype()
        // 这样可以支持 input(FP16) -> output(FP64) 的测试用例

        // 2. 形状检查
        // 无论哪种模式，我们目前假设输入和输出形状必须一致 (Element-wise)
        // 或者已经由前端完成了 Broadcasting 处理，后端只负责处理相同 shape 的计算
        ...
        if (!is_scalar) {
            // 如果是 Tensor 指数，还需要检查指数的形状是否与输入一致
            // (此处简化处理，假设必须完全一致。如果支持广播，逻辑会更复杂)
